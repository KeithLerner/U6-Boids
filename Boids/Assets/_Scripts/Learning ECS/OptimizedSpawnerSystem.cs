...
public partial struct OptimizedSpawnerSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        // Creates a new instance of the job, assigns the necessary data,
        // and schedules the job in parallel.
    ...
    private void Execute([ChunkIndexInQuery] int chunkIndex, ref Spawner spawner)
    {
        ...
        if (spawner.NextSpawnTime < ElapsedTime)
        {
            ...
            Ecb.SetComponent(chunkIndex, newEntity,
                LocalTransform.FromPosition(spawner.SpawnPosition + new float3
                    { x = 0, y = (float)ElapsedTime, z = 0 }));

            // Resets the next spawn time.
            spawner.NextSpawnTime = (float)ElapsedTime + spawner.SpawnFrequency;
        }
    }
}
