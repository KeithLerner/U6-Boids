...
1. EXAMPLE GOAL: EARN 12 STRIKES AND GET KICKED OFF THE TEAM
## Time Invested
```dataviewjs
...
dv.table(
    ...
    dv.pages(`"${folderPath}"`)
        .filter(page => page.Status && page.Status != 'Done' && page.Status != 'Cut')
        .filter(page =>
            Array.isArray(page.Assignee) &&
            page.Assignee.map(a => {
                return getPagePath(a) == getPagePath(dv.current().file.path)
        ...
        .sort(function(a, b) {
            ...
            return b - (numberLookup.indexOf(a.Priority ?? 0))
        })
        .map(page => [
